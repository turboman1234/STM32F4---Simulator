...
void MBMaster(void)
{
   ...
   bytesWritten = OutString(QueryBuffer, MBMasterQueryBufferLenght, USART_3, MB_MASTER_TIMER, T_100_MS);//??????????????????????does timing Ok?????????????????
...
int GetMBAnswer(unsigned char *Buffer)
{
   ...
   //USART_2 must be used   
   BytesRead = InString(Buffer, USART_3, MB_MASTER_TIMER, T_100_MS*100);//??????????????????????does timing Ok?????????????????
