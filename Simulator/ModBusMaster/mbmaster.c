...
void MBMaster(void)
{
   ...
   bytesWritten = OutString(QueryBuffer, MBMasterQueryBufferLenght, USART_2, MB_MASTER_TIMER, T_100_MS);//??????????????????????does timing Ok?????????????????
...
int GetMBAnswer(unsigned char *Buffer)
{
   ...
   BytesRead = InString(Buffer, USART_2, MB_MASTER_TIMER, T_10_MS);//??????????????????????does timing Ok?????????????????
